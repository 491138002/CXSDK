...
public class AlixManager {
	...
	public AlixManager(String DEFAULT_PARTNER, String DEFAULT_SELLER,
			String PUBLIC, String PRIVATE, Activity act, String suject,
			double price, String bill_no, String notify_url,
			OnPayListener onPayListener) {

		this.DEFAULT_PARTNER = DEFAULT_PARTNER;
		this.DEFAULT_SELLER = DEFAULT_SELLER;
		this.PUBLIC = PUBLIC;
	...
	public void init() {
		try {
			...
			String sign = Rsa.sign(info, PRIVATE);
				...
				try {
					...
					if (!result.isSignOk) {
						...
						if (result.resultStatus.equals("9000")) {
							...
							if (onPayListener != null)
								onPayListener.payCallback(true,
										result.resultStatus);
						} else {
							if (onPayListener != null)
								onPayListener.payCallback(false,
										result.resultStatus);
						}
					}
				} catch (Exception e) {
